import { HomeIcon, UserGroupIcon, IdentificationIcon, CalendarIcon } from "@heroicons/react/24/outline";
import { Link, useLocation } from "react-router-dom";
import { useAuth } from "../context/AuthContext";
import { useState } from "react";

function Navbar_Contador() {
    ...
    const menuItems = [
        ...
        { path: "/clients-contador", name: "Clientes", icon: <UserGroupIcon className="w-8"/> },
        { path: "/agenda/contador", name: "Agenda", icon:  <CalendarIcon className="w-8"/> },
        ...
        <nav className={`min-h-screen flex flex-col shadow-lg ${isCollapsed ? 'w-16' : 'w-48'}
            ...
            <ul className={`${isCollapsed ? 'transform duration-200' : 'mt-24 transform duration-200'}`}>
                {menuItems.map((item) => (
                    <li key={item.path}>
                        <Link to={item.path}
                        className={`flex items-center py-4 border-t border-b border-l-0
